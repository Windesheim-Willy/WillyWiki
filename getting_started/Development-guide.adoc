...
You will also need a sufficient coding IDE or text editor. Any one should suffice.
A Github client (eg. SourceTree or Gitkraken) is also necessary in order to "push" and "pull" the existing code from the Github Repository.

Additionally, when troubleshooting code on any of the Raspberry Pi nodes, it might be helpful to look into using `nano` on the Pi directly, if for instance you need to examine the code or make quick edits. More information on this can be found in the section detailing Linux.
...
In most cases, a catkin packages needs to be created via the following commands, this must be done within the catkin workspace;
[source,]
----
cd ~/catkin_ws/
...
An alias for this process is available, simply type the command: `catbmi` (Build, Make, Install) instead, and the above list will be run automatically.
