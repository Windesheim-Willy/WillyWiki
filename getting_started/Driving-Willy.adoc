...
. **Start the ROS Master on the sensor node** +
After the ROS Master has been booted, the ultrasonic sensors and IMU data isn't integrated yet. To visualise these, you will need to start the sensor node from the laptop.
Open a terminal window on the laptop and start an SSH session with the command `ssh ubuntu@sensornode` to remotely access the sensor node Raspberry Pi. Once you're in, invoke the start-up script `./start_sensornode.sh` and look back to RViz to see if the sensors pop up.
...
NOTE: It is necessary to start up the master on the laptop before starting the sensor node, otherwise the footprint will not be visible and the local planner would not function.
