...
|Collecting more sensorinformation. Ie. when Willy drives over a bump this information is collected to analyse if the floor needs maintaince. | X | X | 
|Create multiple smaller Willies that collaborate and drive around on diffrent locations.| X | X | 
...
|Develop sponsoring and donationprogram. Financial funding of Willy needs to improve to be able to invest in newer features.| X | | 
...
|Research navigation in an area with alot of glazing.| X | | 
...
|Extend Willys speech. Introduce multiple voices and languages.| | X | 
|Extend survey options with open or more difficult questions.| X | X | 
|Secure Willys webpage. | | X | 
...
|Benefit of the communication between Social Interaction and Navigation. Ie. drive around if Willy is happy.| | X | 
|===
