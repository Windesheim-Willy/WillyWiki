...
The images of the Ubuntu servers in OVA format can be found in the map "Winnie Skylab\OVA" at SharePoint. The images are the status of December 21, 2018. Some scripts are updated at an later date which can be found at Github.
...
On the Zabbix server (10.10.1.55) Zabbix is installed with default settings. The used user and password can befound in the file "inloggegevens.docx" at SharePoint. Some implementation of servers is done in Zabbix, but is stopped as not being part of the MVP.
