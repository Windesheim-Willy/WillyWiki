...
==== Raspberry Pi
Windesheim has provided us with spare Raspberry Pis. In order to make the physical hardware modulair we used a few Raspberrys. These are all installed by applying each of the defined git repositorys. Installation of each Pi is described for each repository. 
...
IMPORTANT: The Kinect is not being used at this moment.
